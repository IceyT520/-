...
- 混合检索: 向量语义排序 + 化学式精确匹配(`$contains` 过滤) + 结构化数据分支, 经 RRF 加权融合
- 精排: bge-reranker 交叉编码器作为独立"投票者"参与融合(不直接覆盖, 防止通用精排压制领域信号), 同源多样性约束保证引用来源多样
- 多轮对话: 追问自动改写为独立问题("那它的活化能呢?" → "Li3YCl6的活化能是多少?")
- 引用可核验: 网页端可展开查看答案实际依据的原文片段
- 参考文献导出: 一键生成 GB/T 7714 / BibTeX 格式(基于 Crossref 元数据)
- 化学感知查询(超越通用RAG产品的领域能力): "列出所有氧卤化物"、"含In的氯化物有哪些"、"电导率超过5 mS/cm的材料"、材料性能对比表——直接查询人工校准的结构化数据表, 精确无幻觉
- 引用页码定位: 文本块按页切分并记录页码, 引用可定位到原文第 X 页
- 性能排行榜页: 收录材料按室温离子电导率的 Top 15 条形图, 支持按材料类别筛选
- 问答记录导出: 一键下载 Markdown 问答记录(问题/答案/依据片段/GB/T7714参考文献)
- 知识库自生长: 上传新论文后 LLM 自动抽取性能数据进入待确认队列, 人工确认后并入结构化数据表, 数据越用越多
- 新文献追踪: 服务器每周一自动检索 arXiv 卤化物电解质新论文(systemd timer), 网页端可一键下载入库
...
- **PDF解析引擎选型(试验记录)**: 曾评估 pymupdf4llm(Markdown输出, 表格/版面更好), 但正文中的化学式会被拆成 LaTeX 片段(如 `Li$_3$MCl$_6$`), 破坏化学式精确匹配, 召回率从 100% 降至 92%, 故默认仍用 PyMuPDF 纯文本引擎; 如需试验可用 `python src/ingest.py --engine md`。
