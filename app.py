...
- 🔍 **智能检索**：向量语义 + 化学式精确匹配 + Reranker 精排的多路混合检索
- 💬 **多轮对话**：可以追问，如"那它的活化能呢？"，系统自动补全上下文
- 📎 **引用可核验**：每个答案都能展开查看实际依据的原文片段
- 📋 **参考文献导出**：一键生成 GB/T 7714 国标 / BibTeX 格式，写报告直接粘贴
- 🧪 **化学感知查询**：按元素、材料类别、性能阈值精确筛选材料（超越通用 AI 产品的领域能力）
- 📊 **材料对比表**：一句"对比 X 和 Y"直接输出结构化性能对比表
...
离子电导率/活化能/电子电导率/电化学窗口，逐行人工核对）。
它有两个用途：弥补 PDF 表格抽取的天然误差；以及支撑"列举筛选"和"材料对比"两类精确查询。
...
1. **提问**：输入框输入问题回车（或点"提问"），答案流式输出；示例问题可点击填入。
2. **追问**：接着上轮继续问，如"那它的活化能呢？""和 Li3InCl6 比呢？"，系统会自动补全指代。
3. **核验答案**：答案下方展开「📎 本回答依据的文献片段」，查看每个引用对应的原文内容；
   [1][2] 编号与参考文献列表一一对应。
4. **导出参考文献**：回答完成后点「📋 导出本回答的参考文献」，选 GB/T 7714 或 BibTeX 格式，一键复制。
5. **上传文献**：右侧选择 PDF（需上传口令，向管理员索取），提示"已入库"后即可针对它提问。
...
- 系统只处理**文本信息**：论文中的电化学阻抗谱、XRD 图谱等图表数据无法读取，
  遇到这类数据请按引用去查原文
- 列举/对比类答案来自人工校准性能表（目前 55 种材料），未收录的材料会明确提示"暂未收录"，
  可换常规提问从文献中获取
