...
**卤化物固态电解质知识问答系统**——北京化工大学大创项目作品，一个面向卤化物固态电解质（HSSEs）
领域的 RAG 问答助手：你中文提问，系统从文献库检索片段，由 DeepSeek **严格依据片段**生成答案，
并以 [编号] 标注引用。**库里没有的数据，它会说"未找到"，而不是编造。**

## 30 秒上手

1. **提问**：输入框输问题回车（或点示例问题）→ 答案流式输出，可接着追问（"那它的活化能呢？"）
2. **核验**：展开「📎 文献片段」查看答案依据的原文和**页码**
3. **导出**：「📤 导出与下载」里一键复制 GB/T 7714 / BibTeX 参考文献，或下载完整问答记录

## 功能速查

| 想做什么 | 怎么做 | 在哪里 |
|---|---|---|
| 问材料数据/机理 | 直接提问，如"Li3InCl6的电导率是多少？" | 💬 问答页 |
| 追问上一问题 | 直接问"那它呢？""和X比呢？" | 💬 问答页 |
| 按类别/元素/性能筛选材料 | "列出所有氧卤化物""含In的氯化物有哪些""电导率超过5的有哪些" | 💬 问答页（化学感知查询） |
| 对比材料性能 | "对比 Li3YCl6 和 Li3InCl6" → 输出对比表+排序 | 💬 问答页 |
| 看材料性能排行 | 切换标签页，可按类别筛选 | 🏆 性能排行页 |
| 导出参考文献 | 选 GB/T 7714 或 BibTeX 一键复制 | 💬 问答页「📤 导出与下载」 |
| 下载问答记录 | 问题+答案+依据片段+文献打包 .md | 💬 问答页「📤 导出与下载」 |
| 上传自己的文献 | 选 PDF → 上传（需口令）→ 立即可检索 | 📤 上传页 |
| 确认AI抽取的新数据 | 上传后核对数值，点「确认入库」 | 📤 上传页「待确认数据」 |
| 追踪最新论文 | arXiv 每周自动更新，可一键入库 | 📰 新文献页 |
| 查看文献库内容 | 核心库清单/上传列表/总量统计 | 📚 文献库页 |

## 提问建议

- **好问题 = 具体材料 + 具体指标**（"Li3InCl6的室温离子电导率是多少？"），
  大而空的问题（"讲讲固态电池"）效果差
- 化学式直接键盘输入 ASCII 形式（Li3YCl6），无需下标，系统会自动匹配并规范显示
- 一次问一个点；没答好可以换个问法追问

## 数据从哪来

- **核心文献库**：项目组精选 21 篇，覆盖氟/氯/溴/碘化物、氧卤化物、LaCl3 型、界面与合成方法；
  以 Li & Du（*ACS Nano* 2025）系统综述为骨架，含 *Science* 2023/2025 等代表作
- **人工校准性能表**：55 种材料的离子电导率/活化能/电子电导率/电化学窗口，逐行人工核对，
  是化学感知查询、对比表、排行榜的数据地基；通过「知识库自生长」随新论文上传不断扩充
- **用户上传库**：你上传的 PDF 经"抽取→清洗→分块→向量化"入库（所有人可见，勿传涉密文件）
- **新文献**：每周一自动检索 arXiv 卤化物电解质最新论文

## 注意事项

- 系统只读**文本**：电化学阻抗谱、XRD 图谱等图表数据无法获取，请按引用查原文
- 列举/对比/排行来自结构化性能表，未收录的材料会明确提示"暂未收录"；
  其中用户确认扩充的数据由 AI 抽取，建议抽查核对
- **重要数据写入论文/报告前，务必按引用编号核对原文**
- 文献内容仅供科研学习交流（高校图书馆正版订阅 + arXiv 开放获取），请勿再分发；
  答案由 AI 生成，仅供参考

---

北京化工大学大学生创新创业训练计划 · 创新训练项目 | 代码开源：GitHub @IceyT520
遇到异常请联系项目组并附问题截图
"""
